...
export default function UserManagement() {
  ...
  const bulkActivateMutation = useMutation({
    mutationFn: (userIds: string[]) => api.post('/admin/bulk/users/activate', { userIds }),
    onSuccess: () => {
  ...
  const bulkDeactivateMutation = useMutation({
    mutationFn: (userIds: string[]) => api.post('/admin/bulk/users/deactivate', { userIds }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-users'] });
      setSelectedUsers(new Set());
  ...
  const handleBulkAction = (action: BulkAction) => {
    ...
    switch (action.type) {
      case 'activate':
        bulkActivateMutation.mutate(selectedUserIds);
        break;
      case 'deactivate':
        bulkDeactivateMutation.mutate(selectedUserIds);
        break;
      case 'export':
  ...
  return (
    <div className="space-y-6">
      ...
      {showAssignTherapistDialog && assigningUser && (
        <Dialog open={showAssignTherapistDialog} onOpenChange={setShowAssignTherapistDialog}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Assign Therapist to {assigningUser.firstName} {assigningUser.lastName}</DialogTitle>
            </DialogHeader>
            <AssignTherapistForm 
              user={assigningUser}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <select
            title="Role"
            value={formData.role}
            onChange={(e) => setFormData({ ...formData, role: e.target.value as 'user' | 'admin' | 'therapist' })}
...
function EditUserForm({ user, onClose }: { user: User; onClose: () => void }) {
  ...
  const updateUserMutation = useMutation({
    mutationFn: (userData: any) => api.patch(`/users/${user.id}`, userData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-users'] });

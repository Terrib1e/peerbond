...
import { DatabaseService } from '../../../../server/src/services/database';
import { GeminiService } from '../../../../server/src/services/geminiService';

export interface ValidateFeelingsParams {
  memberMessage: string;
  identifiedEmotions?: string[];
  intensity?: 'low' | 'moderate' | 'high' | 'overwhelming';
  context?: 'first_session' | 'ongoing' | 'crisis' | 'breakthrough';
}

/**
 * Validate and normalize the member's emotional experience
 */
export async function validateFeelings(
  params: ValidateFeelingsParams,
  context: ToolContext
): Promise<ToolResult> {
...
Common emotions: sad, angry, anxious, frustrated, hopeful, confused, overwhelmed, lonely, excited, scared, proud, disappointed, grateful, guilty, numb, etc.`;
      ...
      emotions = emotionResponse.split(',').map((e: string) => e.trim().toLowerCase()).filter((e: string) => e.length > 0);
    }
...
Keep the response conversational and genuine. Focus purely on validation, not problem-solving.`;
    ...
    const needsAdditionalSupport = emotions?.some((emotion: string) => 
      ['hopeless', 'suicidal', 'worthless', 'trapped', 'desperate'].includes(emotion)
    ) || params.intensity === 'overwhelming';

...
import { logMood } from '../tools/implementations/logMood';
import { searchGroups } from '../tools/implementations/searchGroups';
import { createActionItem } from '../tools/implementations/createActionItem';
import { analyzeSentiment } from '../tools/implementations/analyzeSentiment';
  ...
  public async callAgentDirectly(
    ...
    try {
      ...
      switch (agentId) {
        ...
        case 'matching':
          // Try to use real searchGroups tool if specific tool requested
          if (toolName === 'searchGroups' || toolName === 'listAllGroups') {
            try {
              const toolContext = {
              ...
              if (searchResult.success && searchResult.data) {
                result = {
                  response: searchResult.data.responseMessage,
                  confidence: 0.9
                };
                toolsUsed = [toolName];
              } else {
                throw new Error('Search tool failed');
              }
            } catch (error) {
              console.error('[Matching] Error using real searchGroups tool, falling back:', error);
              ...
              toolsUsed = ['searchGroups-fallback'];
            }
          } else {
            const matchingResult = await this.matchingAgent(message, session);
            result = {
        ...
        case 'sentiment':
          try {
            ...
            // Use the real analyzeSentiment tool implementation
            const sentimentToolResult = await analyzeSentiment({
              memberMessage: message,
            ...
            if (sentimentToolResult.success && sentimentToolResult.data) {
              const { sentimentScore, sentimentCategory, emotions, crisisRisk, responseMessage } = sentimentToolResult.data;

              result = {
                response: responseMessage || `Sentiment Analysis Results:\n• Sentiment Score: ${sentimentScore}\n• Category: ${sentimentCategory}\n• Emotions: ${emotions.join(', ')}\n• Crisis Risk: ${crisisRisk > 0.7 ? 'High' : crisisRisk > 0.4 ? 'Medium' : 'Low'}`,
                confidence: 0.9
              };

              session.lastSentimentScore = sentimentScore;
              session.crisisLevel = crisisRisk > 0.7 ? 'severe' : crisisRisk > 0.4 ? 'moderate' : crisisRisk > 0.1 ? 'mild' : 'none';
            } else {
            ...
            console.error('[Sentiment] Error using real tool, falling back:', error);
            const sentimentResult = await this.sentimentAgent(message, session);
            result = {
              response: `Sentiment Analysis Results:\n• Sentiment Score: ${sentimentResult.sentimentScore}\n• Crisis Level: ${sentimentResult.crisisLevel}\n• Emotional State: ${sentimentResult.sentimentScore > 0 ? 'Positive' : sentimentResult.sentimentScore < 0 ? 'Negative' : 'Neutral'}`,
              confidence: 0.6
        ...
        case 'tracker':
          result = await this.trackerAgent(message, session);
          toolsUsed = toolName ? [toolName] : ['logMood', 'trackProgress'];
          break;

        case 'action-items':
          result = await this.actionItemsAgent(message, session);
          toolsUsed = toolName ? [toolName] : ['createActionItem', 'getActionItems'];
          break;
...
4. **tracker** - Mood and wellness tracking specialist
   - Tools: logMood, trackProgress, generateMoodReport
   - Use for: Mood logging, emotional state tracking, wellness monitoring
   - When to prefer: "Log my mood", "track my progress", mood check-ins, emotional tracking

5. **action-items** - Goal setting and task management expert
   - Tools: createActionItem, updateActionItem, getActionItems
   - Use for: Setting wellness goals, creating action plans, task management
   - When to prefer: "I want to", "my goal is", "action item", goal setting, improvement plans

6. **crisis** - Emergency intervention (automatically triggered by sentiment analysis)
...
**IMPORTANT**: Aim for HIGH tool utilization - select multiple complementary tools that work together to provide comprehensive support. The goal is engaged, proactive AI assistance.`;
  ...
  private validateAgent(agentName: any): string {
    const validAgents = ['matching', 'facilitator', 'insight', 'tracker', 'action-items'];
  ...
  private getDefaultToolsForAgent(agent: string): string[] {
    const defaultTools = {
      ...
      'tracker': ['logMood', 'trackProgress'],
      'action-items': ['createActionItem', 'getActionItems']
    };
  ...
  private getValidToolsForAgent(agent: string): string[] {
    const agentTools = {
      ...
      'tracker': ['logMood', 'trackProgress', 'generateMoodReport'],
      'action-items': ['createActionItem', 'updateActionItem', 'getActionItems']
    };
  ...
  private getToolDescription(toolName: string): string {
    const toolDescriptions = {
      ...
      // Tracker tools
      'logMood': 'Record member mood and emotional state for tracking and analysis',
      'trackProgress': 'Track wellness progress over time with historical data',
      'generateMoodReport': 'Generate comprehensive mood analysis report',

      // Action Items tools
      'createActionItem': 'Create new wellness goal or action item for member',
      'updateActionItem': 'Update progress on existing action item',
      'getActionItems': 'Retrieve member\'s current action items and goals',

      // Sentiment tools
      'analyzeSentiment': 'Analyze emotional tone and sentiment score of member message',
      'detectCrisis': 'Identify crisis keywords and risk levels',
      'assessEmotionalState': 'Comprehensive emotional state assessment'
    };
  ...
  private getEnhancedFallbackRouting(content: string): {
    ...
    // Mood tracking requests
    const moodKeywords = ['mood', 'feeling good', 'feeling bad', 'track my mood', 'log mood', 'how i feel', 'my emotions'];
    const moodMatch = moodKeywords.some(keyword => lowerContent.includes(keyword));
    const hasMoodContext = lowerContent.includes('track') && (lowerContent.includes('mood') || lowerContent.includes('feeling'));

    if (moodMatch || hasMoodContext) {
      console.log('[ProductionOrchestrator] Fallback: Mood tracking detected');
      return {
        primaryAgent: 'tracker',
        tools: ['logMood', 'trackProgress'],
        reasoning: 'Enhanced fallback: Detected mood tracking request',
        confidence: 0.8
      };
    }

    // Goal setting requests
    const goalKeywords = ['goal', 'action item', 'wellness goal', 'want to', 'need to', 'plan to', 'work on'];
    const goalMatch = goalKeywords.some(keyword => lowerContent.includes(keyword));
    const hasGoalContext = (lowerContent.includes('want to') || lowerContent.includes('need to')) ||
                          (lowerContent.includes('goal') || lowerContent.includes('improve'));

    if (goalMatch || hasGoalContext) {
      console.log('[ProductionOrchestrator] Fallback: Goal setting detected');
      return {
        primaryAgent: 'action-items',
        tools: ['createActionItem', 'getActionItems'],
        reasoning: 'Enhanced fallback: Detected goal setting or improvement request',
        confidence: 0.75
      };
    }
  ...
  private async executeRoutingDecision(
    ...
    try {
      switch (decision.primaryAgent) {
        ...
        case 'tracker':
          console.log('[ProductionOrchestrator] Switching to TRACKER agent');
          const trackerResult = await this.trackerAgent(content, session);
          agentsUsed.push('tracker');
          response = trackerResult.response;
          confidence = Math.max(trackerResult.confidence, decision.confidence);

          // Execute tracker-specific tools
          for (const tool of decision.tools) {
            if (['logMood', 'trackProgress', 'generateMoodReport'].includes(tool)) {
              toolResults.push({
                tool,
                agent: 'tracker',
                executed: true,
                timestamp: new Date(),
                description: this.getToolDescription(tool),
                outcome: `${tool} executed for mood tracking and wellness monitoring`
              });
            }
          }
          break;
        ...
        case 'action-items':
          console.log('[ProductionOrchestrator] Switching to ACTION-ITEMS agent');
          const actionItemsResult = await this.actionItemsAgent(content, session);
          agentsUsed.push('action-items');
          response = actionItemsResult.response;
          confidence = Math.max(actionItemsResult.confidence, decision.confidence);

          // Execute action-items-specific tools
          for (const tool of decision.tools) {
            if (['createActionItem', 'updateActionItem', 'getActionItems'].includes(tool)) {
              toolResults.push({
                tool,
                agent: 'action-items',
                executed: true,
                timestamp: new Date(),
                description: this.getToolDescription(tool),
                outcome: `${tool} executed for goal setting and task management`
              });
            }
          }
          break;
  ...
  /**
   * Tracker Agent - Handles mood logging and progress tracking
   */
  private async trackerAgent(content: string, session: ProductionConversationState): Promise<{
    response: string;
    confidence: number;
  }> {
    try {
      console.log('[TrackerAgent] Processing mood/progress tracking request');
      ...
      // Use the real logMood tool implementation
      const moodResult = await logMood({
        memberMessage: content
      }, toolContext);

      if (moodResult.success && moodResult.data) {
        const { moodScore, emotions, encouragement, trackingStreak } = moodResult.data;

        let response = encouragement || "Thank you for sharing how you're feeling with me.";

        if (trackingStreak && trackingStreak > 1) {
          response += ` 🎉 Amazing! You're on a ${trackingStreak}-day mood tracking streak. This kind of self-awareness is so valuable for your mental health journey.`;
        }

        if (emotions && emotions.length > 0) {
          response += ` I've noted that you're experiencing ${emotions.join(', ')}. These feelings are completely valid.`;
        }

        response += "\n\nWould you like me to help you set any wellness goals based on how you're feeling today?";
        ...
        return {
          response: "I appreciate you wanting to track your mood with me. While I'm having trouble saving it right now, sharing how you feel is always valuable. How has your overall mood been lately?",
          confidence: 0.6
        };
      }
    } catch (error) {
      console.error('[TrackerAgent] Error:', error);
      return {
        response: "I'd love to help you track your mood and progress. How are you feeling today, and what would you like to work on?",
        confidence: 0.5
      };
    }
  }
  ...
  /**
   * Action Items Agent - Handles goal setting and wellness action items
   */
  private async actionItemsAgent(content: string, session: ProductionConversationState): Promise<{
    response: string;
    confidence: number;
  }> {
    try {
      console.log('[ActionItemsAgent] Processing goal/action item request');
      ...
      // Use the real createActionItem tool implementation
      const actionResult = await createActionItem({
        description: content,
      ...
      if (actionResult.success && actionResult.data) {
        const { actionItem, responseMessage, nextSteps } = actionResult.data;

        let response = responseMessage || "Great! I've created a wellness goal for you.";
        ...
        response += "\n\nRemember, small consistent steps lead to meaningful change. Would you like me to help you break this goal down further or set up progress reminders?";
        ...
        return {
          response: "I love that you want to set a wellness goal! While I couldn't save it right now, the intention to improve is already a powerful first step. What specific goal would you like to work toward?",
          confidence: 0.6
        };
      }
    } catch (error) {
      console.error('[ActionItemsAgent] Error:', error);
      return {
        response: "I'd be happy to help you set wellness goals and action items. What would you like to work on or improve in your life?",
        confidence: 0.5
      };
    }
  }
